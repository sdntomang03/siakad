<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-slate-800 leading-tight">Tambah Buku</h2>
    </x-slot>
                    ...
                    <form action="{{ route('books.store') }}" method="POST" class="space-y-4">
                        ...
                        <div>
                            ...
                            <select name="type" id="type_select" class="w-full rounded px-3 py-2 border" required>
                                ...
                                <option value="perpustakaan" {{ old('type')=='perpustakaan' ? 'selected' : '' }}>
                                    Perpustakaan</option>
                            </select>
                        </div>
                        <div id="tingkat_wrapper">
                            ...
                            <select name="tingkat" class="w-full rounded px-3 py-2 border">
                                ...
                                @for($i=1;$i<=6;$i++) <option value="{{ $i }}" {{ old('tingkat')==$i ? 'selected' : ''
                                    }}>{{ $i }}</option>
                                    @endfor
                            </select>
                        </div>
                        ...
                        <div>
                            ...
                            <textarea name="notes" class="w-full rounded px-3 py-2 border">{{ old('notes') }}</textarea>
                        </div>
