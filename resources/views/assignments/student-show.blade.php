    ...
    <div class="py-8 max-w-3xl mx-auto sm:px-6 lg:px-8 space-y-5">
        ...
        @if($assignment->type === 'group')
            <div class="bg-white dark:bg-slate-800 p-6 rounded-2xl border dark:border-slate-700">
                ...
                @if($selectedTask)
                    <div class="mt-5 p-4 rounded-xl bg-emerald-50 border border-emerald-200"><h4 class="font-bold text-emerald-900">{{ $selectedTask->title }}</h4><p class="mt-2 whitespace-pre-line text-sm text-emerald-900">{{ $selectedTask->description }}</p></div>
                @else
