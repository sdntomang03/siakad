        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            <form method="GET" action="{{ route('etpp.dialog-kinerja.index') }}" class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 p-6 flex flex-col sm:flex-row gap-4 items-end">
                ...
                <div class="w-full sm:w-48">
                    ...
                    <select id="dialog-periode" name="periode" class="w-full rounded-lg border-slate-300 dark:border-slate-600 dark:bg-slate-900 dark:text-white">
                ...
                <a id="dialog-pdf-link" href="{{ route('etpp.dialog-kinerja.pdf', ['tahun' => $tahun, 'bulan' => $bulan, 'periode' => $periode]) }}" data-url-template="{{ route('etpp.dialog-kinerja.pdf', ['tahun' => $tahun, 'bulan' => $bulan, 'periode' => '__PERIODE__']) }}" class="px-5 py-2.5 text-indigo-700 bg-indigo-50 rounded-lg font-bold text-sm">Unduh PDF</a>
            </form>
            <script>
                document.getElementById('dialog-periode').addEventListener('change', function () {
                    const pdfLink = document.getElementById('dialog-pdf-link');
                    pdfLink.href = pdfLink.dataset.urlTemplate.replace('__PERIODE__', this.value);
                });
            </script>
